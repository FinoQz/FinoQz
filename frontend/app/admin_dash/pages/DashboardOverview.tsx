...
import { TrendingUp, Users, Clock, DollarSign, CreditCard, PlayCircle, AlertTriangle } from 'lucide-react';
...
import PendingRefundsModal from '../components/adminComponents/PendingRefundsModal';
    ...
    const fetchStats = async () => {
      try {
        ...
        setStats({
          ...
          totalRevenue: 124580,
          totalPaidUsers: 456,
          freeQuizAttempts: 1876,
        });
      } catch (err) {
        console.log('Using default stats - API not available');
        // Keep the default dummy data on error
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 min-h-screen">
      ...
      <div className="mb-6 sm:mb-8 flex items-start justify-between gap-4">
        ...
        {/* Alert Banner */}
        <div className="bg-yellow-50 border-2 border-yellow-200 rounded-xl px-4 py-3 flex items-center gap-2 shadow-sm hover:shadow-md transition cursor-pointer">
          <AlertTriangle className="w-5 h-5 text-yellow-600" />
          <div>
            <p className="text-sm font-semibold text-yellow-900">2 pending refund requests</p>
            <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
        ...
        <div className="bg-white border-2 border-gray-200 rounded-2xl p-4 sm:p-6 hover:shadow-xl transition-all duration-300">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <div className="text-xs sm:text-sm font-medium text-gray-800">Active Users</div>
            <div className="p-2 sm:p-3 bg-green-50 rounded-lg shadow-sm">
              <TrendingUp className="w-5 h-5 sm:w-6 sm:h-6 text-green-600" />
            </div>
          </div>
          <div className="text-2xl sm:text-3xl font-bold text-gray-900">{stats.activeUsers.toLocaleString()}</div>
          <div className="text-xs mt-2 text-green-700">Approved accounts</div>
        </div>

        <div className="bg-white border-2 border-gray-200 rounded-2xl p-4 sm:p-6 hover:shadow-xl transition-all duration-300">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <div className="text-xs sm:text-sm font-medium text-gray-800">Pending Approval</div>
            <div className="p-2 sm:p-3 bg-orange-50 rounded-lg shadow-sm">
              <Clock className="w-5 h-5 sm:w-6 sm:h-6 text-orange-600" />
            </div>
          </div>
          <div className="text-2xl sm:text-3xl font-bold text-gray-900">{stats.pendingApprovals}</div>
        ...
        <div className="bg-white border-2 border-gray-200 rounded-2xl p-4 sm:p-6 hover:shadow-xl transition-all duration-300">
          ...
          <div className="text-2xl sm:text-3xl font-bold text-gray-900">₹{stats.totalRevenue.toLocaleString()}</div>
        ...
        <div className="bg-white border-2 border-gray-200 rounded-2xl p-4 sm:p-6 hover:shadow-xl transition-all duration-300">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <div className="text-xs sm:text-sm font-medium text-gray-800">Total Paid Users</div>
            <div className="p-2 sm:p-3 bg-cyan-50 rounded-lg shadow-sm">
              <CreditCard className="w-5 h-5 sm:w-6 sm:h-6 text-cyan-600" />
            </div>
          </div>
          <div className="text-2xl sm:text-3xl font-bold text-gray-900">{stats.totalPaidUsers}</div>
          <div className="text-xs mt-2 text-cyan-700">Purchased at least one quiz</div>
        </div>

        <div className="bg-white border-2 border-gray-200 rounded-2xl p-4 sm:p-6 hover:shadow-xl transition-all duration-300">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <div className="text-xs sm:text-sm font-medium text-gray-800">Free Quiz Attempts</div>
            <div className="p-2 sm:p-3 bg-indigo-50 rounded-lg shadow-sm">
              <PlayCircle className="w-5 h-5 sm:w-6 sm:h-6 text-indigo-600" />
            </div>
          </div>
          <div className="text-2xl sm:text-3xl font-bold text-gray-900">{stats.freeQuizAttempts.toLocaleString()}</div>
          <div className="text-xs mt-2 text-indigo-700">Total free attempts</div>
        </div>
      </div>

      {/* Charts Section - Updated with new components */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6">
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-200 shadow-lg hover:shadow-xl transition-all duration-300">
          ...
          <div className="h-64">
            {(() => {
              const revenueData = [3200, 4100, 3800, 5200, 6300, 5800, 7100, 6800, 7800, 8200, 7500, 8900, 9200, 8450];
              const maxRevenue = Math.max(...revenueData);
              const days = ['Jan 5', 'Jan 6', 'Jan 7', 'Jan 8', 'Jan 9', 'Jan 10', 'Jan 11', 'Jan 12', 'Jan 13', 'Jan 14', 'Jan 15', 'Jan 16', 'Jan 17', 'Jan 18'];
              ...
              return (
                <div className="h-full flex flex-col">
                  ...
                  <div className="flex justify-between mt-2 pl-2">
                    {days.map((day, index) => (
                      index % 2 === 0 ? (
                        <div key={index} className="text-xs text-gray-600 flex-1 text-center">{day}</div>
                      ) : (
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-200 shadow-lg hover:shadow-xl transition-all duration-300">
          ...
          <div className="h-64">
            {(() => {
              const userData = [980, 1020, 1055, 1090, 1125, 1160, 1180, 1205, 1230, 1250, 1275, 1295, 1320, 1248];
                ...
                <div className="h-full flex flex-col">
                  ...
                  <div className="flex-1 relative border-b-2 border-l-2 border-gray-300 pl-2 pb-2">
                    ...
                    <svg className="w-full h-full" preserveAspectRatio="none">
                      ...
                      {/* Area */}
                      <path
                        d={`M 0,${200 - ((userData[0] - minUsers) / range) * 200} ${userData.map((users, index) => 
                      ...
                      {userData.map((users, index) => (
                        <g key={index}>
                          <circle
                            cx={`${(index / (userData.length - 1)) * 100}%`}
                            cy={200 - ((users - minUsers) / range) * 200}
                            r="4"
                            fill="#22c55e"
                      ...
                      {userData.map((users, index) => (
                        <div key={index} className="flex-1 group relative">
                          <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10"
                               style={{ bottom: `${((users - minUsers) / range) * 200}px` }}>
