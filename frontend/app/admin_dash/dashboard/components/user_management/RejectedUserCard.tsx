...
export default function RejectedUserCard({ user }: RejectedUserCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-white border border-gray-300 rounded-2xl p-4 sm:p-6 hover:shadow-xl transition-all duration-300 cursor-pointer">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-gray-200 to-gray-300 rounded-full flex items-center justify-center shadow-md">
      ...
      {isHovered && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:pointer-events-none">
          ...
          <div className="pointer-events-auto relative bg-white border-2 border-gray-300 rounded-2xl shadow-2xl p-4 sm:p-6 w-full max-w-sm sm:max-w-md animate-in fade-in zoom-in-95 duration-200">
            <button 
            ...
            <div className="space-y-4">
              <div className="pb-3 border-b border-gray-300 pr-8">
                <h4 className="text-lg font-bold text-gray-900 truncate">{user.fullName}</h4>
                <p className="text-xs text-gray-700 font-medium uppercase tracking-wide mt-1 flex items-center gap-1">
              ...
              {/* Rejection Date */}
              <div className="flex items-center gap-3 bg-gradient-to-br from-gray-100 to-gray-200 p-3 rounded-xl shadow-sm">
                <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm">
                  <XCircle className="w-5 h-5 text-gray-800" />
                </div>
                <div>
                  <p className="text-xs text-gray-500 font-medium">Rejection Date</p>
                  <p className="text-sm text-gray-900 font-semibold">{formatDate(user.rejectedAt)}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 bg-gradient-to-br from-gray-100 to-gray-200 p-3 rounded-xl shadow-sm">
                ...
                <div>
                  <p className="text-xs text-gray-500 font-medium">Rejection Time</p>
                  <p className="text-sm text-gray-900 font-semibold">{formatTime(user.rejectedAt)}</p>
                </div>
              </div>
            </div>
          </div>
