        ...
        {exportFormats.map((format, index) => {
          ...
          return (
            <button
              key={index}
              onClick={onExport}
              className="p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition border-2 border-gray-200 hover:border-[var(--theme-primary)] text-left group"
            >
      ...
      <div>
        ...
        <div className="space-y-2">
          {prebuiltReports.map((report, index) => (
            <button
              key={index}
              className="w-full p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition border border-gray-200 hover:border-[var(--theme-primary)] text-left flex items-center justify-between group"
            >
              <div>
                <p className="font-medium text-gray-900 text-sm">{report.label}</p>
                <p className="text-xs text-gray-500">{report.description}</p>
              </div>
              <Download className="w-4 h-4 text-gray-400 group-hover:text-[var(--theme-primary)] transition" />
            </button>
          ))}
        </div>
      </div>
