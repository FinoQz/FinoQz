...
//       const res = await api.post("api/user/signup/verify-mobile", { email, otp });
...
//     try {
//       const email = localStorage.getItem("userEmail");
//       const res = await api.post("api/user/signup/resend-mobile-otp", { email });
...
export default function VerifyMobileOtpPage() {
  ...
  useEffect(() => {
    ...
    if (storedOtp) {
      ...
      setTimeout(() => {
        setTempOtp(null);
        document.cookie = "tempOtp=; Max-Age=0; path=/";
      }, 10000);
    }
  }, []);
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      const newOtp = res.data?.otp;
      if (newOtp) {
        document.cookie = `tempOtp=${encodeURIComponent(newOtp)}; path=/; max-age=10`;
        setTempOtp(newOtp);
        setTimeout(() => {
          setTempOtp(null);
          document.cookie = "tempOtp=; Max-Age=0; path=/";
